...
namespace Phoogle.Tests
{
    ...
    /// Provides methods to easily work with <see cref="PhoogleDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<PhoogleTestModule>
    {
        ...
        {
            void NormalizeDbContext(PhoogleDbContext context)
            {
        ...
        protected void UsingDbContext(Action<PhoogleDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PhoogleDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PhoogleDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PhoogleDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PhoogleDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhoogleDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PhoogleDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhoogleDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PhoogleDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PhoogleDbContext, Task<T>> func)
        {
            T result;
